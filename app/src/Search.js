...
class SearchRender extends React.Component {
	...
	getData = () => {
		...
		axios({
			method: 'post',
			url: 'http://46.101.169.42:4433/search',
			data: {
		...
		return (
			...
			{this.state.status && <div onClick={this.clearstatus} className="alert alert-danger">{this.state.status}</div>}
				...
				{this.state.users.length > 0 && this.state.users.map((user, i) =>
					...
					<img role="presentation" id={i} src={`http://46.101.169.42:4433${user.pictures.length ? user.pictures[0] : '/media/stormtrooper.jpg'}`}/>
					{/* <li style={{color: 'green', fontSize:"20px", display: "inline-block", width: "220px", height: "220px", backgroundImage: `url(http://46.101.169.42:4433${user.pictures.length ? user.pictures[0] : '/media/stormtrooper.jpg'})`}} key={i}>
					{user.username}
					</li> */}
