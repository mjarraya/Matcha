...
class ProfileRender extends React.Component {
	...
	getData = () => {
		...
		axios({
			method: 'get',
			url: 'http://montasar.me:8080/profile',
	...
	upload = async (file) => {
		...
		const response = await axios({
			method: 'put',
			url: 'http://montasar.me:8080/upload',
			data,
			headers: {
	...
	delImage = async (e) => {
		...
		const response = await axios({
			method: 'delete',
			url: 'http://montasar.me:8080/remove',
			data: { imgid },
			headers: { logToken: localStorage.getItem('logToken') },
		});
		this.getData();
	...
	delInterest = async (e) => {
		...
		const response = await axios ({
			method: 'put',
			url: 'http://montasar.me:8080/edit',
				...
				<div className="wrapper">
					{this.state.pictures[0] && (
						...
						<div className="thumbnail">
							<img onDoubleClick={this.delImage} role="presentation" id={0} src={"http://montasar.me:8080" + this.state.pictures[0]} key={0} title="double-click to remove your photo"/>
						</div>
						</div>
					)}
					...
					{this.state.pictures.map((picture, i) => i !== 0 &&
						<div key={i} className="thumbnail">
							<img onDoubleClick={this.delImage} role="presentation" id={i} height="200px" src={"http://montasar.me:8080" + picture} key={i} title="double-click to remove your photo"/>
						</div>
					)}
					</div>
